...
EVIDENCE = ROOT / ".omc/agent-skills"
...
try:
    ...
    assert not (nested / ".agents").exists() and not (repo / ".claude").exists()
    ...
    # then validate EVERY skill in repo, user, and Claude user-copy destinations.
    ...
    paths = [destination / name for destination in (pack, user_pack, mirror) for name in names]
    with ThreadPoolExecutor(max_workers=6) as pool:
        results = list(pool.map(lambda path: run(["npx", "skills-ref", "validate", path]), paths))
    receipt["checks"]["Q3"] = {"passed": all(r.returncode == 0 for r in results), "validated_skills": len(results), "destinations": 3}

    modified = pack / "ulw-work/SKILL.md"
    ...
    assert "ulw-work/SKILL.md" in status["locally_modified"]
