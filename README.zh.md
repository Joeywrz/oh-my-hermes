...
没有任何东西被悄悄记住。候选从会话中捕获，放上评审卡，带着写明的理由被记住、拒绝或推迟。已批准的记录带有来源和复审到期日；确认会重置时钟，沉默会让它从 active 老化到 reference 再到 archive。下一个会话会得到按任务排序、裁剪到 token 预算内的召回包，冲突与重复已被处理。Hermes 自己的记忆从不被读取或修改；这个存储属于 OMH，基于文件，经过评审。当它真的进入某一轮时，Hermes 会在它说话的每个界面上告知：`🧠 OMH — recalled 2 memories`。
...
| `deep` | 强力默认层 | GPT-6 Sol，其次 DeepSeek Flash (V4.1) (high) |
| `architect` | 架构与系统设计 | Claude Fable 5.1，其次 GPT-6 Astra，其次 Kimi K3 (xhigh) |
| `unspecified-high` | 默认工作模型 | Kimi K3，其次 Claude Opus 5.5 (medium) |
| `unspecified-low` | 低成本回退 | GLM 5.3，其次 DeepSeek Flash (V4.1)，其次 Claude Opus 5.5 (low) |
| `quick` | 短任务 | GLM 5.3 Flash，其次 Kimi K3，其次 GPT-6 Luna，其次 Claude Fable 5.1 (low) |
...
| `capable` | 通用高能力工作 | Claude Fable 5.1，其次 Claude Opus 5.5，其次 Kimi K3，其次 GLM 5.3 (medium) |
| `simple-work` | 简单的日常任务 | GPT-6 Luna，其次 DeepSeek Flash (V4.1)，其次 Claude Haiku 4.5 (low) |
...
| 阶段 | 推进事项 |
| --- | --- |
| 理解 | 确认意图、约束、项目术语和停止条件。 |
| 调研 | 用有据可查的产品、代码或运维背景信息取代假设。 |
| 决策 | 明确选项、取舍和决策负责人。 |
| 计划 | 将已确认的范围、代码变更负责人、测试和完成标准整理成可执行计划。 |
| 执行 | 向选定负责人提供边界明确的任务交接，不把准备工作当作实际执行。 |
| 验证 | 依据实际观测到的测试结果、评审结果、CI 结果和运行时证据作出结论。 |
| 运维 | 让发布健康状态、故障事件、回滚状态和后续工作始终清晰可见。 |
| 学习 | 将经过评审、适用范围明确的经验纳入项目记忆，或用于改进工作流。 |
