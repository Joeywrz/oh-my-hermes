...
from ..todo_reconciliation import plan_continuation_directive
...
def pre_verify(
    ...
    **_kwargs: object,
    ...
    messages = [
        text
        for text in (_served_surface_message(paths), _plan_continuation_message(_kwargs, session_id))
        if text
    ]
    if not messages:
        return None
...
def _plan_continuation_message(kwargs: dict[str, object], session_id: str) -> str:
    """The open-plan directive for the session whose turn is ending.
    ...
    and must not fail the turn it is decorating.
    """
    try:
        ...
        return ""
    return plan_continuation_directive(
