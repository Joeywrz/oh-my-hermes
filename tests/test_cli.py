...
"""
    ...
    def test_release_product_readiness_cli_summarizes_product_story(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("211/211 route hints aligned", gates["route_hint_alignment"]["summary"])
            ...
            self.assertIn("248/248 negative-control cases", gates["routing_precision"]["summary"])
            self.assertIn("407/407 interventions", gates["routing_precision"]["summary"])
    ...
    def test_release_evidence_bundle_cli_packages_optional_write_artifact(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn(
                "Routing precision: 248/248 negative controls, 407/407 interventions "
            ...
            self.assertEqual(payload["summary"]["chat_card_coverage_passing"], 91)
            self.assertEqual(payload["summary"]["chat_card_generic_ack_count"], 0)
            self.assertEqual(payload["summary"]["route_hint_alignment_aligned"], 211)
            ...
            self.assertEqual(payload["summary"]["routing_precision_passing"], 248)
            self.assertEqual(payload["summary"]["routing_precision_total"], 248)
            self.assertEqual(payload["summary"]["routing_precision_overroute_count"], 0)
            self.assertEqual(payload["summary"]["routing_precision_intervention_passing"], 407)
            self.assertEqual(payload["summary"]["routing_precision_intervention_total"], 407)
    ...
    def test_demo_chat_card_coverage_keeps_user_facing_routes_dedicated(self) -> None:
        ...
        self.assertEqual(payload["summary"]["case_count"], 91)
        self.assertEqual(payload["summary"]["passing_count"], 91)
    ...
    def test_demo_chat_card_coverage_summary_is_human_readable(self) -> None:
        ...
        self.assertIn("Result: 91/91 workflow cards dedicated (all passing)", stdout)
    ...
    def test_demo_route_hint_alignment_keeps_router_and_awareness_hints_in_sync(self) -> None:
        ...
        self.assertEqual(payload["summary"]["case_count"], 211)
        self.assertEqual(payload["summary"]["hinted_count"], 211)
        self.assertEqual(payload["summary"]["aligned_count"], 211)
        ...
        self.assertEqual(explicit_payload["summary"]["case_count"], 211)
    ...
    def test_demo_route_hint_alignment_summary_is_human_readable(self) -> None:
        ...
        self.assertIn("Result: 211/211 route hints aligned (all passing)", stdout)
        self.assertIn("Hints present: 211/211; missing hints: 0; mismatches: 0", stdout)
        self.assertIn(
