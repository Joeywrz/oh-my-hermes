...
load_local_package()
from omh.install import config_adapter  # noqa: E402
...
from omh.plugin_bundle.omh import delegation_routing  # noqa: E402
...
def _update_config_calls(
    tree: ast.Module, path: Path
) -> list[tuple[str, ast.Call, list[_Scope]]]:
    """Every `update_config` call, with the scope chain it is written in.

    The chain is innermost first and always ends at the module. It exists
    because `src/commands/setup.py` defines `_unregister` twice -- once in
    `_uninstall_hermes_profiles` and once in `cmd_uninstall` -- and a
    module-wide name lookup resolved both call sites to whichever `ast.walk`
    reached last. That silently excused `cmd_uninstall`'s mutation from both
    gates below, which is the one function whose history the gates exist
    for, and made a dirty profile-loop mutation report two sites instead of
    one.
    """
    calls: list[tuple[str, ast.Call, list[_Scope]]] = []

    def walk(node: ast.AST, scopes: list[_Scope]) -> None:
        if isinstance(node, ast.Call):
            func = node.func
            target = func.attr if isinstance(func, ast.Attribute) else getattr(func, "id", "")
            if target == "update_config":
                calls.append((f"{path.relative_to(REPO_ROOT)}:{node.lineno}", node, scopes))
        inner = [node, *scopes] if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)) else scopes
...
def definitions_in_scope(scope: _Scope) -> dict[str, ast.FunctionDef | ast.AsyncFunctionDef]:
    """Functions defined directly in `scope`, not inside a function nested in it."""
...
def resolve_in_scope(
    name: str, scopes: list[_Scope]
) -> ast.FunctionDef | ast.AsyncFunctionDef | None:
    """The function `name` binds to, innermost scope first."""
    for scope in scopes:
        match = definitions_in_scope(scope).get(name)
...
class UpdateConfigDisciplineTests(unittest.TestCase):
    ...
    def test_a_timeout_from_the_mutation_names_the_mutation_not_the_lock(self) -> None:
        """`FileLockTimeout` in this repo is a `TimeoutError`.

        Wrapping the whole loop in `except TimeoutError` reported any future
        mutation that touches a locked store as a lock timeout on the
        config. Narrowing that handler to the acquisition fixed the label
        and left the exception escaping as a traceback, which is not a
        refusal either. It is refused now, and the message says which
        timeout it was.
        """
        with TemporaryDirectory() as tmp:
            home = _Home(Path(tmp))
            ...
            with self.assertRaises(ConfigMutationFailed) as caught:
            ...
            self.assertIn("the config mutation timed out", str(caught.exception))
            self.assertNotIn("lock", str(caught.exception))
            self.assertIsInstance(caught.exception.__cause__, TimeoutError)

    def test_any_other_exception_from_the_mutation_belongs_to_the_caller(self) -> None:
        """Only `TimeoutError` is reclassified; everything else propagates.

        `ensure_external_dir` raises `ValueError` on a shape it refuses and
        `omh setup` maps that to its own error, which predates this
        function. Swallowing it here would take that mapping away.
        """
        with TemporaryDirectory() as tmp:
            home = _Home(Path(tmp))
    ...
    def test_the_backoff_between_rounds_grows_and_is_jittered(self) -> None:
        """The jitter has a reason recorded; this is what holds it.

        Timing cannot be asserted without flakiness, so the sleep is a seam
        and the test reads the delays it was asked for. Without this,
        deleting the sleep left all twenty other cases green.
        """
        with TemporaryDirectory() as tmp:
            home = _Home(Path(tmp))
            delays: list[float] = []
            ...
            with self.assertRaises(ConfigConcurrentUpdate):
                update_config(
                    home.config,
                    _always_races,
                    omh_home=home.omh,
            ...
            # One pause between rounds, never after the last.
            self.assertEqual(len(delays), 3)
            self.assertTrue(all(delay > 0 for delay in delays))
            # Jittered, so each round's window is bounded rather than exact.
            for attempt, delay in enumerate(delays, start=1):
                self.assertGreaterEqual(delay, 0.005 * attempt * 0.5)
                self.assertLessEqual(delay, 0.005 * attempt * 1.5)

    def test_one_attempt_is_reported_in_the_singular(self) -> None:
        with TemporaryDirectory() as tmp:
            home = _Home(Path(tmp))

            def _always_races(text: str) -> ConfigChange:
                home.config.write_text(text + "# x\n", encoding="utf-8", newline="")
                return ensure_external_dir(text, "/tmp/omh/skills")

            with self.assertRaises(ConfigConcurrentUpdate) as caught:
                update_config(home.config, _always_races, omh_home=home.omh, attempts=0)

            self.assertIn("during 1 update attempt:", str(caught.exception))

    def _symlinked_config(self, home: "_Home") -> Path:
    ...
    def _assert_symlink_refused(self, home: "_Home", real: Path) -> None:
        # `ConfigUnwritable`, not `ConfigConcurrentUpdate`: nothing raced
        # here, and an exception that named a race would put a cause in the
        ...
        self.assertEqual(real.read_text(encoding="utf-8"), BASE_CONFIG)
    ...
    def test_a_symlinked_config_is_refused_rather_than_replaced_with_a_file(self) -> None:
        with TemporaryDirectory() as tmp:
            home = _Home(Path(tmp))
            self._assert_symlink_refused(home, self._symlinked_config(home))

    @unittest.skipUnless(hasattr(os, "symlink"), "needs symlink support")
    def test_a_symlinked_config_is_refused_where_o_nofollow_does_not_exist(self) -> None:
        """The Windows shape, modelled so it runs on every platform.

        `read_config_snapshot` refuses a symlink by opening with
        `O_NOFOLLOW` and catching the `OSError`. `os.O_NOFOLLOW` is
        Unix-only, so on Windows `getattr(os, "O_NOFOLLOW", 0)` makes the
        flag a no-op, the open follows the link and succeeds, and the
        refusal never fires. Worse, the compare's two sides then stop
        describing one file: the read stamps `fstat` on the followed
        descriptor (the target) and the guard stamps `lstat` on the path
        (the link). Measured on macOS with the flag forced to 0, the two
        differed in inode and size, every round mismatched, and the command
        failed with `config.yaml kept changing during 3 update attempts` --
        the exact error the `test-windows` job produced.

        Setting `os.O_NOFOLLOW = 0` reproduces that composition here,
        because it is the same `getattr` the reader performs.
        """
        with TemporaryDirectory() as tmp:
            home = _Home(Path(tmp))
            real = self._symlinked_config(home)
            original = getattr(os, "O_NOFOLLOW", 0)
            try:
                os.O_NOFOLLOW = 0  # type: ignore[attr-defined]
                self._assert_symlink_refused(home, real)
            finally:
                if hasattr(os, "O_NOFOLLOW"):
                    os.O_NOFOLLOW = original  # type: ignore[attr-defined]

    @unittest.skipUnless(hasattr(os, "symlink"), "needs symlink support")
    def test_the_symlink_refusal_precedes_the_compare_rather_than_following_it(self) -> None:
        """Mutation proof, and the reason the two stat calls may differ.

        With the refusal removed, the reader is the only thing standing in
        the way, and on a host without `O_NOFOLLOW` it lets the link
        through. The fix is to keep symlinks away from the compare, not to
        make both sides follow links: following on both sides would stop a
        file swapped for a symlink between the read and the replace from
        being detected at all.
        """
        with TemporaryDirectory() as tmp:
            home = _Home(Path(tmp))
            self._symlinked_config(home)
            reads: list[Path] = []
            real_reader = delegation_routing.read_config_snapshot

            def _follows(path: Path):
                reads.append(path)
                return real_reader(Path(os.path.realpath(path)))

            with mock.patch.object(config_adapter, "read_config_snapshot", _follows, create=True):
            ...
            # Refused before the reader ran at all, so no compare happened.
...
class CallSiteGateTests(unittest.TestCase):
    ...
    def test_nothing_writes_a_hermes_config_path_expression_a_level_lower(self) -> None:
        ...
        What it sees, stated because it is narrower than it sounds: the
        destination EXPRESSION must mention `hermes_config_path`. That
        catches `atomic_write_text(paths.hermes_config_path, text)`, which
        is how every call site in this repo spells it at the point of the
        write. It does not catch a path first bound to a local -- which is
        the repo's other habit, as `system/targets` does with
        `config_path = Path(str(target["hermes_config_path"]))` -- because
        answering that needs dataflow, not a name. A reviewer still owns
        that spelling.
        """
...
class MutationShapeGateTests(unittest.TestCase):
    ...
    def _mutations(self) -> list[tuple[str, ast.AST]]:
        ...
        for path in sorted(SRC.rglob("*.py")):
            tree = ast.parse(path.read_text(encoding="utf-8"), filename=str(path))
            for where, node, scopes in _update_config_calls(tree, path):
                ...
                if isinstance(mutation, ast.Lambda):
                    ...
                    continue
                resolved = (
                    resolve_in_scope(mutation.id, scopes) if isinstance(mutation, ast.Name) else None
                )
                found.append((where, resolved if resolved is not None else mutation))
        return found

    def test_a_name_resolves_to_the_definition_in_its_own_scope(self) -> None:
        """Two functions can share a name; the gate must read the right one.

        `src/commands/setup.py` defines `_unregister` twice, because the
        uninstall command and the bot-profile loop take the same shared
        helper. A module-wide lookup resolved both call sites to whichever
        `ast.walk` reached last, so the gates below examined the profile
        loop's body twice and the uninstall command's never -- excusing the
        one function whose history they exist for.
        """
        source = """
def outer_a():
    def shared(text):
        return A(text)
    update_config(path, shared, omh_home=home)
...
"""
        ...
        resolved = {
            where.rsplit(":", 1)[1]: resolve_in_scope(node.args[1].id, scopes)
            for where, node, scopes in _update_config_calls(tree, SRC / "probe.py")
        }

        self.assertEqual(len(resolved), 2)
        bodies = {line: ast.unparse(match.body[0]) for line, match in resolved.items()}
        self.assertEqual(sorted(bodies.values()), ["return A(text)", "return B(precomputed)"])
